CREATE OR REPLACE PACKAGE BODY CRM_PROYECTOS_API AS
  PROCEDURE PROYECTOS_LIST(p_result OUT SYS_REFCURSOR, p_x01 IN NUMBER DEFAULT NULL) IS
  BEGIN
    OPEN p_result FOR
  ...
  PROCEDURE PROYECTOS_GET(p_result OUT SYS_REFCURSOR, p_x01 IN NUMBER) IS
  BEGIN
    OPEN p_result FOR
      SELECT p.id, p.nombre, p.descripcion, p.estado,
             p.oportunidad_id, o.nombre AS oportunidad_nombre,
  ...
  PROCEDURE PROYECTOS_CREATE(
    ...
    p_x01 IN VARCHAR2, p_x02 IN CLOB DEFAULT NULL, p_x03 IN NUMBER DEFAULT NULL
  ) IS
    v_id NUMBER;
  BEGIN
  ...
  PROCEDURE PROYECTOS_UPDATE(
    ...
    p_x04 IN VARCHAR2 DEFAULT NULL, p_x05 IN NUMBER DEFAULT NULL
  ) IS
  BEGIN
    UPDATE crm_proyectos
       SET nombre = p_x02, descripcion = p_x03,
           estado = p_x04, oportunidad_id = NVL(p_x05, oportunidad_id)
     WHERE id = p_x01;
    COMMIT;
  ...
  PROCEDURE PROYECTOS_ESTADO(p_result OUT SYS_REFCURSOR, p_x01 IN NUMBER, p_x02 IN VARCHAR2) IS
  BEGIN
