-- ============================================================
-- CRM EXTRUCOL - PACKAGE BODY
-- One statement at a time in SQL Workshop
-- ============================================================
...
CREATE OR REPLACE PACKAGE BODY CRM_USUARIOS_API AS
  ...
  BEGIN
    OPEN p_result FOR
      SELECT id, nombre, email, rol,
             ciudad_id, telefono, activo, fecha_creacion
        FROM crm_usuarios ORDER BY nombre;
  END;

  PROCEDURE USUARIOS_GET(p_result OUT SYS_REFCURSOR,
                         p_x01 IN NUMBER) IS
  BEGIN
    OPEN p_result FOR
      SELECT id, nombre, email, rol,
  ...
  BEGIN
    INSERT INTO crm_usuarios
      (nombre, email, contrasena, rol, ciudad_id)
