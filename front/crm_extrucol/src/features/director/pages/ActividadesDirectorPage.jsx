...
export default function ActividadesDirectorPage({ esDirector = true }) {
  ...
  useEffect(() => {
    if (esDirector) directorAPI.usuarios().then(u => setUsuarios(u.filter(x => x.rol === 'EJECUTIVO'))).catch(() => {})
    cargar({ inicio, fin })
  }, [])
  ...
  // Filtrar por ejecutivo y agrupar por día
    ...
    <AppLayout>
      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row gap-2 mb-4 flex-wrap">
          ...
          {/* Filtro por ejecutivo (solo director CE-35) */}
        ...
        {/* CE-32: agrupadas por día */}
        {!loading && porDia.map(([dia, acts]) => (
          <div key={dia} className="mb-4">
            <div className="text-[12px] font-bold text-[#ABABAB] uppercase tracking-wider mb-2 capitalize">
              {formatDate(dia + 'T12:00:00')}
              <span className="ml-2 text-[11px] font-semibold px-1.5 py-0.5 rounded-full bg-[#EEF1FA] text-[#24388C]">
                {acts.length}
              </span>
            </div>
            <div className="flex flex-col gap-2">
              {acts.map(a => (
                <div key={a.id} className="bg-white rounded-xl border border-[#F0F0F0] shadow-sm px-5 py-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        {a.resultado && (
                          <div className="text-[12px] italic text-[#4A4A4A] bg-[#F7F7F7] rounded px-2 py-1 mt-1">{a.resultado}</div>
                        )}
                          ...
                          {a.oportunidad?.nombre && (
                            <span className="flex items-center gap-1">
                              <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M20.25 14.15v4.25c0 1.094-.787 2.036-1.872 2.18A48.23 48.23 0 0112 20.25c-2.291 0-4.545-.16-6.378-.42C4.787 19.681 4 18.739 4 17.645V13.4" />
                              </svg>
                              {a.oportunidad.nombre}
                            </span>
                          )}
                          <span>{formatDateTime(a.fecha_actividad)}</span>
                          <span className={`px-1.5 py-0.5 rounded text-[10.5px] font-semibold ${a.virtual ? 'bg-[#F0F0F0] text-[#6B6B6B]' : 'bg-[#EEF1FA] text-[#24388C]'}`}>
